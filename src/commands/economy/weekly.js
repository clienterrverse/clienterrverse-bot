...
export default {
  ...
  run: async (client, interaction) => {
    ...
    const now = Date.now();
    const timeSinceLastWeekly =
      now - (userBalance.lastWeekly ? userBalance.lastWeekly.getTime() : 0);
    if (userBalance.lastWeekly && timeSinceLastWeekly < weeklyCooldown) {
      ...
      return interaction.reply({
        content: `You have already claimed your weekly reward. Please try again in ${days} days, ${hours} hours, and ${minutes} minutes.`,
        ephemeral: true,
      });
    }
