...
export default {
  data: new SlashCommandBuilder()
    .setName('triggered')
    .setDescription("applies a triggered filter to a user's avatar")
    .addUserOption((option) =>
      option
        .setName('user')
        .setDescription('user whose avatar you want to apply the filter to:')
        .setRequired(false)
    )
    .setContexts([0, 1, 2])
  ...
  run: async (client, interaction) => {
    try {
      ...
      const attachment = new AttachmentBuilder(img).setName('rainbow.gif');
