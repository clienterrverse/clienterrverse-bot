...
export default {
  data: new SlashCommandBuilder()
    ...
    .setDescription("Applies a 'Double Stonks' effect using two users' avatars")
  ...
  run: async (client, interaction) => {
    try {
      ...
      const img = await new DIG.DoubleStonk().getImage(avatarUrl1, avatarUrl2);

      // Create an attachment using AttachmentBuilder
      const attachment = new AttachmentBuilder(img).setName('doublestonks.png');
