...
async function handleListSubcommand(client, interaction) {
    ...
    client.guilds.cache.map(async (guild) => {
      ...
      try {
        const channels = guild.channels.cache.filter(channel => 
          channel.type === 0 && channel.permissionsFor(guild.members.me).has(PermissionFlagsBits.CreateInstantInvite)
        );
        if (channels.size > 0) {
  ...
  const embeds = createServerListEmbeds(client, interaction, guilds, isDetailed);
...
async function handleLeaveSubcommand(client, interaction) {
  ...
  if (!guild) {
    return await interaction.editReply(`I am not in a server with the ID ${serverId}.`);
  }
  ...
  try {
    ...
    if (confirmation.customId === 'confirm_leave') {
      await guild.leave();
      await confirmation.update({ content: `I have left the server **${guild.name}** (ID: ${serverId}).`, components: [] });
    } else {
      await confirmation.update({ content: 'Server leave cancelled.', components: [] });
    }
  } catch (error) {
    await interaction.editReply({ content: 'No response received, cancelling server leave.', components: [] });
  }
}
...
async function handleUserSubcommand(client, interaction) {
  ...
  const userServers = client.guilds.cache.filter(guild => guild.ownerId === user.id);
  const serverCount = userServers.size;

  const serverList = userServers.map(guild => 
    `- ${guild.name} (ID: ${guild.id}, Members: ${guild.memberCount})`
  ).join('\n');

  const embed = new EmbedBuilder()
    .setTitle(`Servers Owned by ${user.username}`)
    .setDescription(`User ${user.username} (ID: ${user.id}) owns **${serverCount}** server(s) that the bot is in.`)
    .setColor(mconfig.embedColorDefault)
    .addFields({ name: 'Server List', value: serverList || 'No servers found.' })
    .setFooter({
