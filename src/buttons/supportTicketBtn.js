...
async function handleButtonTicket(interaction, ticketSetup) {
  ...
  if (!category) {
    return await interaction.reply({
      content: "The ticket category no longer exists. Please contact an administrator.",
      ephemeral: true
    });
  }
  ...
  if (!staffRole) {
    return await interaction.reply({
      content: "The staff role no longer exists. Please contact an administrator.",
      ephemeral: true
    });
  }
  ...
  const ticketChannel = await createTicketChannel(guild, category, member, staffRole, ticketCount);
  ...
  const newTicket = await createTicketDocument(guild, member, ticketChannel, interaction.channel);
  ...
  if (ticketSetup.logChannelID) {
    ...
    if (logChannel) {
      await logChannel.send(`New ticket created by ${member.user.tag} (${member.id}) - <#${ticketChannel.id}>`);
    }
  }
}

async function createTicketChannel(guild, category, member, staffRole, ticketCount) {
...
function createTicketEmbed(member, guild) {
  return new EmbedBuilder()
    ...
    .setDescription("A staff member will be with you shortly. Please explain your issue in as much detail as possible.")
    .addFields(
...
async function createTicketDocument(guild, member, ticketChannel, parentChannel) {
